...
-(void)selectTimeView:(CJSelectTimeScrollView *)timeScroll didSelectYearAtIndexPath:(NSIndexPath *)indexPath cellText:(NSString *)cellText{
    ...
    if (is2M29D) {
        gregorainToChinese = [self.useTime timeChineseCalendarWithString:[NSString stringWithFormat:@"%@-%@-%i",cellText, self.timeView.monthText, [self.timeView.dayText integerValue] - 1]];
